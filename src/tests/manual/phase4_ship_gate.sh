#!/usr/bin/env bash
# phase4_ship_gate.sh — driver for the Phase 4 ship-gate fixtures.
#
# Runs each scenario under
# src/tests/fixtures/phase4_ship_gate/scenarios/ and asserts the
# Phase 4 plan §5 + PR-PHASE4-* done-when criteria.
#
# Usage:
#   phase4_ship_gate.sh [--mode=<hook-sim|real>] [--scenario=<name>] [--keep]
#
# Modes:
#   --mode=hook-sim   (default) drives via direct hook invocations and
#                     uses the init.sh fake claude binary that scripts
#                     validator + Mediator outputs via env. No real
...
FIXTURE_ROOT="$SRC_ROOT/tests/fixtures/phase4_ship_gate"
...
if [ "$MODE" = "real" ]; then
  printf 'phase4_ship_gate: real-mode deferred to Phase 7 integration harness\n'
  exit 77
fi
...
printf '== phase4_ship_gate driver — mode=%s ==\n\n' "$MODE"
...
printf '\n'
printf '== phase4_ship_gate: %s pass, %s fail (mode=%s)\n' "$PASS_COUNT" "$FAIL_COUNT" "$MODE"
