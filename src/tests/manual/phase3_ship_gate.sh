#!/usr/bin/env bash
# phase3_ship_gate.sh — driver for the Phase 3 ship-gate fixtures.
#
# Runs each scenario under
# src/tests/fixtures/phase3_ship_gate/scenarios/ and asserts the
# Phase 3 plan §5 done-when criteria.
#
# Usage:
#   phase3_ship_gate.sh [--mode=<hook-sim|real>] [--scenario=<name>] [--keep]
#
# Modes:
#   --mode=hook-sim   (default) drives via direct hook invocations and
#                     synthesizes Mediator verdict files inline. No
#                     real claude -p spawn. Deterministic.
...
FIXTURE_ROOT="$SRC_ROOT/tests/fixtures/phase3_ship_gate"
...
if [ "$MODE" = "real" ]; then
  printf 'phase3_ship_gate: real-mode deferred to Phase 7 integration harness\n'
  exit 77
fi
...
printf '== phase3_ship_gate driver — mode=%s ==\n\n' "$MODE"
...
for scenario_dir in $(ls -1d "$SCENARIOS_ROOT"/*/ 2>/dev/null | sort); do
  ...
  # Source the shared init helper to mint a fresh workspace.
  ...
  if [ "$KEEP_WORKDIR" = "0" ]; then
    sleep 0.2   # drain any remaining background jobs (F-016 family)
...
printf '\n'
printf '== phase3_ship_gate: %s pass, %s fail (mode=%s)\n' "$PASS_COUNT" "$FAIL_COUNT" "$MODE"
